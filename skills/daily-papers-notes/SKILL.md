...
2. **质量检查已有笔记**（不是只看文件是否存在）：
   - 对已有 `📄 **快速报告**` 或 `📒 **深度笔记**` 标记的论文，用 Glob 找到对应报告文件
   - quick 报告必须包含 `## 元信息`、`## 一句话总结`、`## 核心贡献`、`## 问题背景`
   - deep 报告行数 >= 120 且包含 `## 关键公式`、`## 关键图表`、`## 实验`
   - 不达标的视为骨架报告，必须重新生成
3. 对每篇需要生成/重新生成的论文，使用 Task agent 调用 `/paper-reader` skill（传入 arXiv 链接）
   - 必读传入“深度阅读/完整解析”，值得看传入“快速看一下/review”
   - **不要指定固定的输出路径**，让 paper-reader 根据 `{YYYY}/{来源}/{basename}` 规则自行决定文件名和分类目录
   - paper-reader 会用方法名缩写作为文件名（如 `DAPL.md`），并自动归档到 `论文笔记/{年份}/{来源}/`
...
3. 包含 `![` 图片引用（至少 1 张）
4. 包含 `## 关键公式`、`## 关键图表` 和 `## 实验` section header
5. 如果任一条件不满足，**删除文件并重新生成**

每篇 quick 报告生成后，立即验证：
1. 包含 `report_mode: quick`
2. 只要求 `## 元信息`、`## 一句话总结`、`## 核心贡献`、`## 问题背景`
3. 文件位于 `论文笔记/{年份}/{来源}/`
...
- 如果该论文已有 `📄 **快速报告**`、`📒 **深度笔记**`、`📒 **已有笔记**` 或 `📒 **笔记**` 行，跳过不重复添加
