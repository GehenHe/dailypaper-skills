...
默认论文精读采用 `assets/paper-note-template.md` 中的中等完整标准：关键方法图、核心公式、主结果/消融证据和知识关系优先。本文件中的“所有 Figure/Table/公式”要求只用于用户明确要求“完整图表提取”“零遗漏精读”或需要审计论文全部实验细节时。
...
完整提取模式下，提取论文中的所有表格（ALL Tables），完整保留所有行列数据：
...
完整提取模式下，每篇论文笔记包含所有 Figure，一张都不能漏。
...
完整提取模式遵循零遗漏原则：所有图、所有公式、所有表格全部出现在笔记中。
